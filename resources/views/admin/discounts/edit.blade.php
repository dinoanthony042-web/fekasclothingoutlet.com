                            ...
                            <div>
                                ...
                                <div class="ml-6 space-y-2">
                                    ...
                                    <label class="inline-flex items-center text-sm">
                                        <input type="checkbox" id="apply_all_checkbox" name="apply_all_categories" value="1" onchange="toggleApplyAll()" class="h-4 w-4 text-indigo-600 border-gray-300 rounded" {{ old('apply_all_categories', $discount->apply_all_categories) ? 'checked' : '' }}>
                                    ...
                                    <select name="category_ids[]" id="category_multi_select" multiple
                                        ...
                                        @foreach($categories as $category)
                                            <option value="{{ $category->id }}" {{ (is_array(old('category_ids', $discount->category_ids ?? [])) && in_array($category->id, old('category_ids', $discount->category_ids ?? []))) ? 'selected' : '' }}>{{ $category->name }}</option>
                                            @foreach($category->children as $subcategory)
                                                <option value="{{ $subcategory->id }}" {{ (is_array(old('category_ids', $discount->category_ids ?? [])) && in_array($subcategory->id, old('category_ids', $discount->category_ids ?? []))) ? 'selected' : '' }}>&nbsp;&nbsp;{{ $subcategory->name }}</option>
                                            @endforeach
                                        @endforeach
                                    </select>
