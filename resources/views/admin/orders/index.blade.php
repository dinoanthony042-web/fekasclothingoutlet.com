...
<div class="space-y-4 sm:space-y-6">
    ...
    <div class="bg-white rounded-2xl shadow-sm p-4 sm:p-6">
        <form method="GET" action="{{ route('admin.orders.index') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4">
            <div>
            ...
            <div>
                <label class="block text-xs sm:text-sm font-medium text-gray-700 mb-1">Sale Type</label>
                <select name="sale_type" class="w-full rounded-xl border border-gray-300 bg-gray-50 px-3 py-2 text-sm text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200">
                    <option value="">All Sales</option>
                    <option value="online" {{ request('sale_type') === 'online' ? 'selected' : '' }}>Online</option>
                    <option value="walk_in" {{ request('sale_type') === 'walk_in' ? 'selected' : '' }}>Walk-in</option>
                </select>
            </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Sale Type</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50 transition duration-150">
                            ...
                            <td class="px-4 py-3 text-sm">
                                @if(($order->shipping_address['delivery_method'] ?? null) === 'walk_in')
                                    <span class="inline-flex items-center rounded-full bg-green-100 px-2.5 py-1 text-xs font-semibold text-green-800">Walk-in</span>
                                @else
        ...
        <div class="lg:hidden space-y-3">
            ...
            <div class="bg-white rounded-2xl border border-gray-200 p-4 sm:p-5 shadow-sm">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 mt-4 text-sm">
                    ...
                    <div>
                        <p class="text-gray-500 text-xs">Sale Type</p>
                        <p class="font-semibold {{ ($order->shipping_address['delivery_method'] ?? null) === 'walk_in' ? 'text-green-700' : 'text-blue-700' }}">
                            {{ ($order->shipping_address['delivery_method'] ?? null) === 'walk_in' ? 'Walk-in' : 'Online' }}
                        </p>
                    </div>
                </div>
