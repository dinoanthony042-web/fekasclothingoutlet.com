...
<div class="mx-auto max-w-xl rounded-[2rem] border border-[#e6d9f5] bg-white p-10 shadow-[0_20px_50px_-30px_rgba(0,0,0,0.14)]">
    ...
    <form id="login-form" method="post" action="{{ $loginAction ?? route('login.store') }}" class="space-y-6">
        ...
        <div>
            <label class="block text-sm font-semibold text-[#4f433d]">Email or Username</label>
            <input type="text" name="login" value="{{ old('login') }}" required class="mt-3 w-full rounded-3xl border border-[#e6d9f5] bg-[#faf5ff] px-4 py-3 text-sm outline-none focus:border-[#5b1e7e] focus:ring-4 focus:ring-[#5b1e7e]/10" />
        </div>
        <div>
        ...
        <div class="flex items-center justify-between text-sm text-[#6e625d]">
            ...
            <a href="{{ route('password.request') }}" class="font-semibold text-[#5b1e7e] hover:text-[#e91e8c]">Forgot password?</a>
        </div>
